...
const PricingCard = ({ plan }) => (
  <div className={`flex flex-col w-104 p-4 border rounded-lg shadow-md ${plan.bestValue ? 'bg-white border-gray-300' : ''}`} style={{ height: '500px' }}>
    <h2 className="font-bold text-2xl mb-0">{plan.name}</h2>
    {plan.worldprice && <p className="text-gray-500 text-sm">{`World Price: ${plan.worldprice}`}</p>}
    {plan.price && <p className="font-bold text-2xl my-2 text-center">{plan.price}</p>}
    <p className="mb-2 overflow-hidden text-ellipsis" style={{ display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>{plan.description}</p>
    <ul className="mb-2 overflow-hidden text-ellipsis" style={{ display: '-webkit-box', WebkitLineClamp: 6, WebkitBoxOrient: 'vertical' }}>
      {plan.features.map((feature, index) => (
        <li key={index} className="flex items-center mb-1 border-b pb-1">
          <span>{feature}</span>
        </li>
      ))}
    </ul>
